...
public partial class Backstage_company_ShowScenic : System.Web.UI.Page
{
    ...
    protected void GV_ShowScenic_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        ...
        int ScenicID =Convert.ToInt32(GV_ShowScenic.DataKeys[e.RowIndex].Value.ToString());
    ...
    protected void GV_ShowScenic_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        int ScenicID =Convert.ToInt32(GV_ShowScenic.DataKeys[e.RowIndex].Value.ToString());
        sqlString = "delete  from TB_SJP where ScenicID=" + ScenicID;
        int x = db.ExecuteSQL(sqlString);
        sqlString = "delete  from TB_Scenic where ScenicID=" + ScenicID;
        int y = db.ExecuteSQL(sqlString);
        if(x>0&&y>0)
        {
            Response.Write("<script>alert('删除景点成功');</script>");
            bind();
        }
